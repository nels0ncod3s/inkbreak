...
INKBREAK is a Three.js FPS rendered like a living notebook drawn in blue ballpoint pen. This build contains the arena roguelite loop, movement systems, weapon progression, enemy reactions, dynamic page geometry, environmental interactions, boss encounter, four Story chapters, and a responsive landscape mobile control layer.
...
### Chapter Three — The Draftworks
Enter the repair workshop beneath the page graph, restore three Line Anchors, uncover evidence that The Artist was reconstructing damage, survive an Eraser Storm, and face THE REDACTOR before crossing the Source Gate.
...
## Mobile firing + HUD layout pass

- Mobile now supports concurrent movement, free-look and firing with independent touch IDs.
- FIRE can be held and dragged to rotate the camera while automatic fire continues.
- AIM/ADS can also be dragged as an aiming trackpad.
- Settings expose separate mobile normal-look and firing-look sensitivity multipliers, plus a FIRE/ADS drag-aim toggle.
- Settings > HUD Layout includes 2-finger, 3-finger and 4-finger presets, Save, Reset and an interactive HUD editor.
- The HUD editor supports drag positioning, 50–200% scale, 20–100% opacity, grid snapping and safe-edge snapping.
- Layouts serialize to `localStorage` using normalized viewport percentages so they survive resolution/orientation changes.
- Safe-area clamps account for notches/home indicators and are reapplied on viewport resize/orientation changes.

## Collision / sky / Chapter Three pass (2026-09-11)

### Collision

- Player horizontal movement now uses a swept/sub-stepped controller path with a maximum movement slice smaller than the capsule radius, preventing dash/sprint tunneling through thin walls.
- Static visual boxes keep the loose hand-drawn rotation, while physical collision uses thick primitive AABBs with minimum depth for wall-like geometry.
- Hard depenetration first restores the last known legal position, then performs minimum-translation pushback, then falls back to a small authored-space radial recovery.
- A first-person camera safety probe prevents the eye point from remaining inside wall geometry. Because INKBREAK is first-person, the probe pushes the controller/camera out rather than moving an independent third-person camera and desynchronizing hitscan aim.
- Camera near plane increased from 0.05 to 0.08 to reduce near-surface visual precision artifacts.

### Sky / fog

- Replaced the flat background color with a generated equirectangular paper-sky panorama. Desktop uses a 2048×1024 canvas; lower mobile tiers use 1024×512.
- Clouds, horizon strokes and paper color are baked into the single sky texture.
- Distance blend uses `FogExp2` plus one low-cost horizon haze cylinder that follows the player in X/Z.
- The existing unlit paper materials remain unchanged; no real-time lights or shadows were added, preserving mobile GPU cost and the notebook aesthetic.

### Chapter Three — The Draftworks

- Added Chapter Three to Chapter Select and sequential progression.
- Added a lazy-built Draftworks map region with workshop bays, drafting tables, three Line Anchors, the Draft Foundry and Source Gate.
- New progression: Repair Bench → three anchor encounters → Eraser Storm movement beat → Draft Foundry reveal → THE REDACTOR boss → Source Gate.
- Added THE REDACTOR enemy profile and a mid-fight Layout Purge that redraws low cover at ~55% health.
- Added three optional environmental notes that deepen the Artist / Borrowed Line / Mara mystery.
- Full level design documentation lives in `CHAPTER-3-LDD.md`.

## Visual clarity + iOS gesture safety pass

- Fixed a graphics-manager bug where manual QUALITY could still be dynamically reduced toward a low DPR. Manual Performance/Balanced/Quality modes are now fixed-resolution caps; only AUTO can scale dynamically.
- New render ceilings: Performance 1.0x mobile / 1.15x desktop, Balanced 1.5x mobile / 1.6x desktop, Quality up to 2.0x. AUTO starts sharp, waits through a warm-up period, then uses multi-window hysteresis before lowering or raising render resolution.
- Added an advanced Render Scale control for manual presets and expanded the F3 performance overlay with screen/canvas resolution, device DPR, render DPR, and scale.
- Medium/high touch devices now keep antialiasing enabled; low-tier devices may disable it for GPU/thermal headroom.
- iOS/PWA controls now reserve an additional bottom gesture-safe margin above the Home Indicator. HUD serialization clamps saved controls out of that area, and the HUD editor visibly shades the unsafe strip.
- Touches beginning inside the protected bottom OS gesture strip are ignored by gameplay. This cannot disable iOS' Home gesture, but it prevents INKBREAK from asking players to perform normal gameplay gestures directly on top of it.
- Orientation/resize now recalculates renderer dimensions, pixel ratio, safe areas, and mobile HUD placement after Safari has updated its viewport.
